...
export function toNotchianYaw(yaw: number) {
  toDegrees(PI - yaw);
}
export function toNotchianPitch(pitch: number) {
  toDegrees(-pitch);
}
export function fromNotchianYawByte(yaw: number) {
  fromNotchianYaw(yaw * FROM_NOTCH_BYTE);
}
export function fromNotchianPitchByte(pitch: number) {
